...
void CLaserOdometry2DNode::publish()
{
  ...
  // rf2o không tự ước lượng độ bất định của phép đo -- để covariance mặc định
  // 0 hết (như code gốc) khiến bộ lọc Kalman phía sau (vd robot_localization
  // EKF) hiểu nhầm là "chắc chắn tuyệt đối", có thể ghi đè state một cách sai
  // lệch. Điền giá trị cố định hợp lý: tin x/y/yaw/vx/vyaw (đo trực tiếp từ so
  // khớp scan), coi z/roll/pitch/vy/vz/vroll/vpitch là không đo được (variance
  // lớn). Nên tinh chỉnh lại theo thực nghiệm khi có robot thật.
  constexpr double kPosVar = 0.01;      // ~10cm std cho x,y
  constexpr double kYawVar = 0.02;      // ~8 độ std cho yaw
  constexpr double kUnobservedVar = 1e6;
  constexpr double kLinVelVar = 0.01;   // ~10cm/s std cho vx
  constexpr double kAngVelVar = 0.02;   // ~8 độ/s std cho vyaw

  odom.pose.covariance = {
    kPosVar, 0, 0, 0, 0, 0,
    0, kPosVar, 0, 0, 0, 0,
    0, 0, kUnobservedVar, 0, 0, 0,
    0, 0, 0, kUnobservedVar, 0, 0,
    0, 0, 0, 0, kUnobservedVar, 0,
    0, 0, 0, 0, 0, kYawVar,
  };
  odom.twist.covariance = {
    kLinVelVar, 0, 0, 0, 0, 0,
    0, kUnobservedVar, 0, 0, 0, 0,
    0, 0, kUnobservedVar, 0, 0, 0,
    0, 0, 0, kUnobservedVar, 0, 0,
    0, 0, 0, 0, kUnobservedVar, 0,
    0, 0, 0, 0, 0, kAngVelVar,
  };
